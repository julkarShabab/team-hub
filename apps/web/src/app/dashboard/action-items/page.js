...
export default function ActionItemsPage() {
  ...
  const { items, isLoading, fetchItems, updateItemStatus, deleteItem } = useActionItemStore();
  ...
  const ItemCard = ({ item, compact = false }) => (
    <div
      ...
      className={`card p-3 cursor-pointer hover:shadow-md transition-all ${item._optimistic ? 'opacity-60' : ''} ${
    ...
    >
      <div className="flex items-start gap-2">
        <div className="flex-1 min-w-0">
          <p className={`font-medium text-gray-900 dark:text-white ${compact ? 'text-sm' : ''}`}>
            {item.title}
          </p>
          {item.goal && (
        ...
        <div className="flex flex-col items-end gap-1 flex-shrink-0">
          <span className={PRIORITY_CONFIG[item.priority]?.class || 'badge-neutral'}>
      ...
      <div className="flex items-center justify-between mt-2">
        ...
        {item.dueDate && (
          <div className={`flex items-center gap-1 text-xs ${
            new Date(item.dueDate) < new Date() && item.status !== 'DONE' ? 'text-red-500' : 'text-gray-400'
          }`}>
            <Calendar size={10} />
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1 bg-gray-100 dark:bg-gray-800 rounded-lg p-1">
            ...
            >
              <Columns size={16} className={view === 'kanban' ? 'text-brand-500' : 'text-gray-400'} />
            </button>
            <button
              onClick={() => setView('list')}
              className={`p-1.5 rounded ${view === 'list' ? 'bg-white dark:bg-gray-700 shadow-sm' : ''}`}
            >
              <List size={16} className={view === 'list' ? 'text-brand-500' : 'text-gray-400'} />
            </button>
          </div>
        ...
        <div className="flex gap-4 flex-1 overflow-x-auto pb-4">
          {STATUSES.map((status) => {
            ...
            return (
              ...
              >
                {/* Column header */}
                <div className={`flex items-center justify-between p-3 rounded-t-xl ${STATUS_COLORS[status]}`}>
                  <div className="flex items-center gap-2">
                ...
                {/* Cards */}
                <div className={`flex-1 min-h-40 p-2 space-y-2 rounded-b-xl bg-gray-50 dark:bg-gray-900/50 overflow-y-auto`}>
                  {colItems.map((item) => (
              ...
              {items.map((item) => (
                ...
                >
                  <td className="py-3 px-4">
                    ...
                    {item.goal && <p className="text-xs text-brand-500">↳ {item.goal.title}</p>}
                  </td>
                  <td className="py-3 px-4">
                    <span className={
                      item.status === 'DONE' ? 'badge-success' :
                      item.status === 'IN_PROGRESS' ? 'badge-info' : 'badge-neutral'
                    }>
                  ...
                  <td className="py-3 px-4">
                    {item.assignee ? (
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-gray-700 dark:text-gray-300">{item.assignee.name}</span>
                      </div>
                    ) : (
