...
import {
  ...
  Users, BarChart3, LogOut, Settings, Plus, ChevronDown,
  Moon, Sun, Bell
} from 'lucide-react';
...
export default function DashboardLayout({ children }) {
  ...
  const { user, isLoading, refreshUser, logout, isAuthenticated } = useAuthStore();
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-gray-50 dark:bg-gray-950">
      {/* Sidebar */}
      <aside className={`${sidebarOpen ? 'w-64' : 'w-16'} flex-shrink-0 bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 flex flex-col transition-all duration-200`}>
        {/* Workspace Switcher */}
            ...
            <div className="mt-1 card overflow-hidden shadow-lg z-50 absolute w-56">
              {workspaces.map((ws) => (
                ...
                >
                  ...
                  <span className="truncate text-gray-700 dark:text-gray-300">{ws.name}</span>
