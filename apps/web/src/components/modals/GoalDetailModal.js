...
import { CheckCircle, Circle, Plus, Edit3, TrendingUp } from 'lucide-react';
...
export default function GoalDetailModal({ goalId, onClose, userRole = 'MEMBER' }) {
  const { currentGoal, fetchGoal, updateMilestone, addProgressUpdate, addMilestone } = useGoalStore();
  ...
  const overallProgress = currentGoal?.milestones?.length
    ? Math.round(currentGoal.milestones.reduce((s, m) => s + m.progress, 0) / currentGoal.milestones.length)
    : 0;
    ...
    <Modal title={currentGoal.title} onClose={onClose} size="lg">
      <div className="p-5 space-y-6">
        ...
        {currentGoal.description && (
          <p className="text-gray-600 dark:text-gray-300 text-sm">{currentGoal.description}</p>
        )}
          ...
          {showMilestoneInput && (
            <div className="flex gap-2 mb-3">
              <input
                ...
                autoFocus
              />
              <button onClick={handleAddMilestone} className="btn-primary text-sm px-3">Add</button>
            </div>
          )}
        ...
        <div>
          ...
          <div className="flex gap-2 mb-4">
            ...
            />
            <button onClick={handleAddProgress} disabled={submitting} className="btn-primary text-sm px-3">
              Post
            </button>
          </div>
          ...
          <div className="space-y-3 max-h-48 overflow-y-auto">
              ...
              <div key={u.id} className="flex gap-3">
                ...
                <div className="flex-1 bg-gray-50 dark:bg-gray-800 rounded-lg px-3 py-2">
                  <div className="flex items-center justify-between mb-1">
                    <p className="text-xs font-medium text-gray-700 dark:text-gray-300">{u.user?.name}</p>
                    <p className="text-xs text-gray-400">{format(new Date(u.createdAt), 'MMM d')}</p>
                  </div>
            ...
            {!currentGoal.progressUpdates?.length && (
              <p className="text-sm text-gray-400 text-center py-3">No updates yet. Be the first to post!</p>
            )}
          </div>
        </div>
      </div>
