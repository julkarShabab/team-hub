...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      emitToWorkspace(req.app.get('io'), req.workspaceId, SOCKET_EVENTS.ANNOUNCEMENT_CREATED, full);
...
router.put(
  ...
  async (req, res, next) => {
    try {
      ...
      emitToWorkspace(req.app.get('io'), existing.workspaceId, SOCKET_EVENTS.ANNOUNCEMENT_UPDATED, announcement);
...
router.delete('/:announcementId', authenticate, async (req, res, next) => {
  try {
    ...
    emitToWorkspace(req.app.get('io'), existing.workspaceId, SOCKET_EVENTS.ANNOUNCEMENT_DELETED, {
  ...
  try {
    ...
    if (existingReaction) {
      ...
      reaction = await prisma.reaction.create({
        data: { announcementId: req.params.announcementId, userId: req.user.id, emoji },
      });
    }
    ...
    emitToWorkspace(req.app.get('io'), existing.workspaceId, SOCKET_EVENTS.REACTION_ADDED, {
      announcementId: req.params.announcementId,
      reactions,
    });
...
// ─── Comment ──────────────────────────────────────────────────────────────────
router.post('/:announcementId/comments', authenticate, async (req, res, next) => {
  try {
    ...
    for (const mention of mentions) {
      ...
      const mentioned = await prisma.user.findFirst({ where: { name: { contains: name } } });
      if (mentioned && mentioned.id !== req.user.id) {
        ...
        req.app.get('io').to(`user:${mentioned.id}`).emit(SOCKET_EVENTS.NOTIFICATION, {
          type: 'MENTION',
          message: `${req.user.name} mentioned you in a comment`,
        });
      }
    ...
    emitToWorkspace(req.app.get('io'), existing.workspaceId, SOCKET_EVENTS.COMMENT_ADDED, {
      announcementId: req.params.announcementId,
      comment,
    });
